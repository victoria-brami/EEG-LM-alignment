...
def extract_same_time_window_rows(
    window_start: int, window_end: int, tab: pd.DataFrame
):
    extract_tab = tab[
        (tab["truncate_start"] == window_start) & (tab["truncate_end"] == window_end)
    ]
    return extract_tab


def extract_same_electrode_rows(electrodes: Union[list, str], tab: pd.DataFrame):
...
def get_representations(inputs: Union[list, torch.tensor], model: Callable) -> np.array:
    model.eval()
    hidden = []
...
def get_model_representations(
    inputs: Union[List[str], List[dict], torch.Tensor],
    model: AutoModel,
    layer: int = -1,
    ...
    Args:
        inputs (Union[List[str], List[dict], torch.Tensor]): _description_
        model (Callable): _description_
        layer (int, optional): _description_. Defaults to -1.
        tokenizer (Callable, optional): _description_. Defaults to None.
    ...
    with torch.no_grad():
        if layer == -1:
            hidden_states = [
                model(**inputs[i], output_hidden_states=False)
            ...
            hidden_states = [
                model(**inputs[i], output_hidden_states=True)
                .hidden_states[layer]
                .cpu()
                .numpy()[:, 0]
                for i in range(len(inputs))
...
def compute_kmeans_labels(
    features: np.array, n_clusters: int, mode: str = "normal"
) -> np.array:
...
def compute_all_representations_distances(
    features, list_paired_indices, norm="l2", normalize=True
):
    distances = []
    for id1 in np.unique(list_paired_indices[:, 0]):
        id2 = list_paired_indices[
            np.where(np.array(list_paired_indices)[:, 0] == id1)[0], 1
        ]
        if norm == "cosine":
            ...
            if normalize:
                ...
                dist = np.linalg.norm(
                    np.repeat(features[id1][None, :], len(id2), axis=0) - features[id2],
                    axis=1,
                )
...
def compute_all_optimized_representations_distances(
    features, list_paired_indices, norm="l2", normalize=True
):
    distances = []
    ...
    if norm == "cosine":
        ...
        distance_matrix = (
            np.abs(
                -2 * np.dot(normed_features, normed_features.T)
                + np.sum(normed_features**2, axis=1)
                + np.sum(normed_features**2, axis=1)[:, np.newaxis]
            )
            ** 0.5
        )
    ...
    mask = np.array(
        np.ones((len(features), len(features))) - np.eye(len(features)), dtype=bool
    )
...
def compute_correlations(
    ...
    for start_trunc in tqdm(
        range(0, eegs.shape[-1] - timesteps + 1, pad_step),
        desc="Computing correlations for each time range",
    ):
        start = time()
        ...
        for chan_id in range(len(list_electrodes)):
            ...
            row_dict = {
                "Channel": f"Channel {save_chan_id} ",
                "distance": None,
            ...
            cosine_distances = compute_all_optimized_representations_distances(
                reshaped_eegs, list_paired_indices, norm="cosine"
            )
            l2_distances = compute_all_optimized_representations_distances(
                reshaped_eegs, list_paired_indices
            )
