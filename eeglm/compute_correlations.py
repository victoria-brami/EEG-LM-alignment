...
def main(config):
    ...
    list_words = [
        dataset[i]["word"] for i in range(len(dataset))
    ]  # getattr(dataset, "list_words")
    ...
    config.tab_name = "_".join(
        [rep_name, str(config.timesteps) + "ms", config.tab_name]
    )
    ...
    if config.word_distance == "levenshtein":
        ...
        word_features = get_model_representations(
            list_words, model, config.model.layer, tokenizer
        )
    ...
    eeg_signals = np.stack(
        [dataset[i]["raw_eeg_input_ids"] for i in range(len(dataset))]
    )
