...
from eeglm.dataset import get_dataset_electrodes, project_3d_coordinates_in_plan
...
def main(config):
    ...
    data_path_name = (
        "eeg_POS" if config.data.dataname == "ubira" else config.data.dataname
    )
    ...
    list_results_grouped_tabs = [
        corr.extract_sub_table(
            attribute="distance", value=config.distance, groupby_key="truncate_start"
        )
    ...
    for subtitle_id in tqdm(
        range(len(fig_sub_titles)), desc="Generating Correlation plots"
    ):
        # Reshape the plots
        pears_corr_val = split_into_chunks(
            pears_corr_values[subtitle_id], config.vis.chunk_size
        )
        spear_corr_val = split_into_chunks(
            spear_corr_values[subtitle_id], config.vis.chunk_size
        )
        ...
        plot_2d_topomap(
            ...
            rows=n_rows,
            size=config.vis.size,
            cols=n_cols,
            edgecolor=config.vis.edgecolor,
