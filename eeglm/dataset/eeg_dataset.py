...
from eeglm.dataset.base import BaseDataset
from eeglm.dataset.constants import BrainMEGElectrodes
from eeglm.utils import parse_table_labels, read_table
...
def _split_sent_data_into_words_data(
    start_id: int,
    sent_id: str,
    eeg_sig: np.array,
...
def get_dataset_electrodes(
    rootpath: str, dataname: str
...
def project_3d_coordinates_in_plan(
    coords: np.array,
) -> np.array:
    COEF = 0.4
...
class UBIRADataset(BaseDataset):
    ...
    def _load_sentence_labels(self):
        self.sentence_labels = pd.read_csv(
            os.path.join(self.datapath, "sentences_indices.csv")
        )
    ...
    def _get_sentence_wise_data(
        self, sent_id: str, mode: str = "average", power: bool = True
    ) -> List:
        ...
        if mode == "average":
            ...
            if not power:
                ...
                eeg_pows_arr = eeg_pows_arr.reshape(
                    -1, channels, n_words, timesteps
                ).transpose(0, 2, 1, 3)
            ...
            if not power:
                ...
                # Apply the Power computation for each trial
                eeg_pows_arr = _compute_eeg_power_signal(eeg_sigs_arr)
                eeg_pows_arr = eeg_pows_arr.reshape(-1, n_words, timesteps).transpose(
                    1, 0, 2
                )
    ...
    def _load_data(self, mode: str = "word", use_power: bool = True):
        ...
        for sent_i, row in tqdm(
            self.sentence_labels.iterrows(), total=self.config.n_sentences
        ):
            if sent_i > self.config.n_sentences:
                break
            sent_id_name = row["common_id"]

            _, eeg_sig, labels_df = self._get_sentence_wise_data(
                sent_id_name, mode="average", power=use_power
            )

            if mode == "word":
                word_data_list = _split_sent_data_into_words_data(
                    self.start_id + sent_i,
                    sent_id_name,
                    eeg_sig,
                    labels_df,
...
class KilowordDataset(BaseDataset):
    ...
    def _load_labels(self):
        ...
        if self.filter_labels == "none":
            ...
            self.labels_df = parse_table_labels(
                self.labels,
                LIST_LABELS,  # Here self.filter_labels must be LIST_LABELS
        ...
        if self.filter_labels is not None:
            if self.filter_labels == "OBJECT":
                ...
                all_ids = self.labels_df[
                    self.labels_df[self.filter_labels] is True
                ].index
        else:
    ...
    def _load_data(self):
        ...
        eeg_data = read_table(
            os.path.join(self.datapath, "KWORD_ERP_LEXICAL_DECISION_DGMH2015.csv")
        )
...
class HarryPotterMEGDataset(BaseDataset):
    ...
    def _load_labels(self):
        ...
        if "PRON+AUX" in self.filter_labels:
            with open(
                os.path.join(self.datapath, "sentences_and_pos_extended.json"), "r"
            ) as json_file:
                self.sent_data = json.load(json_file)
        else:
            with open(
                os.path.join(self.datapath, "sentences_and_pos.json"), "r"
            ) as json_file:
                self.sent_data = json.load(json_file)
        with open(
            os.path.join(self.datapath, "split_sentences_and_pos.json"), "r"
        ) as json_f:
    ...
    def _load_channels(self):
        ...
        self.info._set_channel_positions(
            self.channels,
            self.channel_names,
        )
    ...
    def _load_data(self):
        ...
        for fold in self.config.list_folders:
            ...
            if self.config.sampling_rate != 1000:
                ...
                # shape (N, )
                subject_meg_data = h5py.File(
                    os.path.join(subject_path, subject_file[0])
                )
            ...
            for sent_id in range(len(self.split_sent_data)):
                ...
                for i, word_id in enumerate(self.split_sent_data[sent_id]["word_ids"]):
                    ...
                    # Apply Specific label filtering if needed
                    if (
                        self.filter_labels is not None
                        and sample["pos"] not in self.filter_labels
                    ):
                        continue
                    self.data.append(sample)
...
def get_dataset(
    config: Union[Dict, OmegaConf], tokenizer: AutoTokenizer, dataname: str
):
