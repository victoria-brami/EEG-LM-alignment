...
def parse_table_labels(
    table: pd.DataFrame, list_labels: list, labelcolname: str = "SEMANTIC_FIELD"
):
    labels = table[[labelcolname]]
    if labelcolname == "SEMANTIC_FIELD":
        labels = labels.fillna("")
        tab_labels = pd.DataFrame(
            {k: labels[labelcolname].apply(lambda x: k in x) for k in list_labels}
        )
    else:
        tab_labels = pd.DataFrame(
            {labelcolname: labels[labelcolname].apply(lambda x: x == "YES")}
        )
    return tab_labels
...
def extract_correlations_and_periods(grouped_tab):
    ...
    for i, (state, frame) in enumerate(grouped_tab):
        ...
        sub_titles.append(
            f"{int(float(state) * 1000 / 256)} to {int(frame['truncate_end'].values[0] * 1000 / 256)} ms"
        )

    return pears_corr_values, spear_corr_values, sub_titles


def get_model(model_id: str, load_random: bool = False):
    ...
    if load_random:
        # Initialize a model with the same configuration but with random weights
