...
def vis_parser():
    ...
    parser.add_argument(
        "--model", type=str, default="canine_s", help="Name of the Language Model used"
    )
    parser.add_argument(
        "--label_name", type=str, default="OBJECT", help="Name of the Label used"
    )
...
def build_source_folder(
    table_path: str = "", distance_type: str = "l2", corr_type: str = "spearman"
):
    pass
...
def main(config):
    ...
    if config.data.dataname == "harry_potter":
        list_electrodes = [
            list_electrodes[i] for i in range(0, len(list_electrodes), 3)
        ]
    ...
    list_corr_names = [
        filename
        for filename in os.listdir(corr_save_folder)
        if model in filename
        and "random" not in filename
        and "layer" in filename
        and f"{config.timesteps}ms" in filename
    ]
    ...
    reordered_list_corr_names = [
        (int(name.split(f"layer_")[1].split("_")[0]), name) for name in list_corr_names
    ]
    ...
    pears_dest_file_path = build_destination_folder(
        list_corr_tabs[-1].table_path,
    ...
    spear_dest_file_path = build_destination_folder(
        list_corr_tabs[-1].table_path,
        config.data.dataname,
        config.save_folder,
        config.distance,
    ...
    for subtitle_id in range(len(fig_sub_titles)):
        # Reshape the plots
        pears_corr_val = split_into_chunks(
            pears_corr_values[subtitle_id], config.chunk_size
        )
        spear_corr_val = split_into_chunks(
            spear_corr_values[subtitle_id], config.chunk_size
        )
        ...
        plot_2d_topomap(
            ...
            dpi=200,
            title=fig_sub_titles[subtitle_id],
            savepath=pears_dest_file_path
            + f"/pearson_{model}_{fig_sub_titles[subtitle_id]}.png",
        )
        ...
        plot_2d_topomap(
            ...
            + f"/spearman_{model}_{fig_sub_titles[subtitle_id]}.png",
        )
