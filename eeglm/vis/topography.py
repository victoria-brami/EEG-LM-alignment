...
def _prepare_kiloword_topomap(
    ...
    **fig_kwargs,
    ...
    nose = Polygon(
        [
            electrodes[4]
            + np.array([abs(electrodes[4, 0] - electrodes[15, 0]) / 3, 0.0]),
    ...
    if (rows, cols) in [(1, 1), (None, None)]:
        ...
        if axs is None:
            fig, ax = plt.subplots(
                rows, cols, figsize=(size * cols, size * rows), **fig_kwargs
            )
        else:
            ax = axs
        for j in range(cols):
        ...
        if axs is None:
            ...
            ax_fris = fig.add_subplot(
                rows + 1, 1, rows + 1, adjustable="box", aspect=12, anchor=(-0.25, 0.15)
            )
            ...
            for j in range(cols):
                ...
                nose = Polygon(
                    [
                        electrodes[4]
                        + np.array(
                            [abs(electrodes[4, 0] - electrodes[15, 0]) / 3, 0.0]
                        ),
                        electrodes[15] + np.array([0, 0.02]),
                        electrodes[24]
                        - np.array(
    ...
    **fig_kwargs,
    ...
    if (rows, cols) in [(1, 1), (None, None)]:
        ...
        nose = Polygon(
            [
                electrodes[0]
                + np.array([abs(electrodes[0, 0] - electrodes[34, 0]) / 3, 0.0]),
            ...
            for j in range(cols):
                ...
                nose = Polygon(
                    [
                        electrodes[0]
                        + np.array(
                            [abs(electrodes[0, 0] - electrodes[34, 0]) / 3, 0.0]
                        ),
...
def _prepare_hp_topomap(
    ...
    **fig_kwargs,
    ...
    if (rows, cols) in [(1, 1), (None, None)]:
        left_ear = Ellipse(
            electrodes[54] - np.array([head_radius / 12, 0]),
            height=head_radius / 4,
            width=head_radius / 8,
            facecolor="none",
            **fig_kwargs,
        ...
        right_ear = Ellipse(
            electrodes[98] + np.array([head_radius / 12, 0]),
            height=head_radius / 4,
            width=head_radius / 8,
            facecolor="none",
            **fig_kwargs,
        ...
        nose = Polygon(
            [
                [electrodes[28, 0], electrodes[28, 1] + head_radius / 6],
        ...
        for j in range(cols):
            ...
            ellipse = Ellipse(
                head_center
                + np.array(
                    [0, (np.max(electrodes[:, 1]) + np.min(electrodes[:, 1])) / 2 + 0.2]
                ),
                height=head_radius,
                width=head_radius,
                facecolor=col,
            ...
            for j in range(cols):
                ...
                ellipse = Ellipse(
                    ...
                    + np.array(
                        [
                            0,
                            (np.max(electrodes[:, 1]) + np.min(electrodes[:, 1])) / 2
                            + 0.2,
                        ]
                    ),
                    height=head_radius,
...
def _prepare_topomap(
    ...
    **fig_kwargs,
    ...
    if dataname == "kiloword":
        ...
        return _prepare_kiloword_topomap(
            electrodes, head_radius, cols, rows, size, axs=axs, **fig_kwargs
        )
    elif dataname == "ubira":
        head_radius = np.linalg.norm(electrodes[25, :2] - electrodes[8, :2])
        return _prepare_ubira_topomap(
            electrodes, head_radius, cols, rows, size, axs=axs, **fig_kwargs
        )
...
def plot_2d_topomap(
    ...
    **fig_kwargs,
    ...
    fig, ax, ax_fris = _prepare_topomap(
        coords2d,
        dataname,
        rows=rows,
        cols=cols,
    ...
    if (rows, cols) in [(1, 1), (None, None)]:
        ...
        contour = ax.contourf(
            grid_x, grid_y, grid_z, levels=15, cmap=cmap, vmin=-0.3, vmax=0.3
        )
        ...
        for j in range(cols):
            ...
            ax[j].scatter(
                coords[:, 0], coords[:, 1], c=values[j], edgecolors="k", cmap=cmap
            )
        ...
        fig.colorbar(list_contours[0], ax=ax, orientation="horizontal", fraction=0.1)
        ...
        for i in range(rows):
            for j in range(cols):
                ...
                grid_x, grid_y = np.meshgrid(
                    np.linspace(
                        coords2d.min() - margin, coords2d.max() + margin, grid_res
                    ),
                    np.linspace(
                        coords2d.min() - margin, coords2d.max() + margin, grid_res
                    ),
                ...
                grid_z = griddata(
                    coords2d[:, :2], values[i][j], (grid_x, grid_y), method="cubic"
                )
        ...
        ax_fris.broken_barh(
            [(0, 500), (time_range[0], time_range[1] - time_range[0])],
            (0, 2),
        ...
        fig.tight_layout()
        fig.colorbar(list_contours[0], ax=ax, orientation="vertical", fraction=0.05)
